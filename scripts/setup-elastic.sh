...
delete_all_slos_by_name() {
    ...
    if [[ -n "$slo_ids" ]]; then
        local count
        count=$(echo "$slo_ids" | grep -c . || echo 0)
        if [[ $count -gt 0 ]]; then
            print_info "Deleting ${count} existing SLO(s): ${name}"
            # Delete all SLOs in parallel for speed
            echo "$slo_ids" | while IFS= read -r slo_id; do
                if [[ -n "$slo_id" ]]; then
                    curl -s --max-time 10 -X DELETE "${KIBANA_URL}/api/observability/slos/${slo_id}" \
                        -H "Authorization: ApiKey ${ELASTIC_API_KEY}" \
